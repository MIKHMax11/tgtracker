...
(function() {
  ...
  const DEFAULT_DATA = {
    ...
    investments: [], // Track investments separately
    categories: {
  ...
  class AppState {
    ...
    loadData() {
      try {
        ...
        const data = stored ? JSON.parse(stored) : DEFAULT_DATA;
  ...
  const TransactionsTable = {
    ...
    deleteTransaction(id) {
      const transaction = state.data.transactions.find(t => t.id === id);
      const investmentIndex = state.data.investments ? state.data.investments.findIndex(i => i.id === id) : -1;
      
      if (!transaction && investmentIndex === -1) return;
